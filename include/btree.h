...
#define BTREE_UNLOCK(tree, flags) \
	(lockmgr(&(tree)->bt_lock, LK_RELEASE | flags, 0))
...
#define BUCKET_HASNEXT(node) \
	((*(bnode_ptr *)fnode_getval(node, NODE_MAX(node))) != 0)
...
int fbtree_init(struct vnode *backend, bnode_ptr ptr, fb_keysize keysize,
    fb_valsize valsize, compare_t comp, char *, uint64_t fbflags,
    struct fbtree *tree);
...
int fbtree_iterat(
    struct fbtree *tree, const void *key, struct fnode_iter *iter);
...
int fnode_create_bucket(
    struct fnode *node, int at, void *key, struct fnode **fin);
