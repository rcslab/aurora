...
static int
allocate_membuf(void **buf, size_t *alloc_size, vm_map_t vmmap) {
    *alloc_size = sizeof(struct vmspace_info);
    vm_map_entry_t entry;
    ...
    for (entry = vmmap->header.next; entry != &vmmap->header; entry = entry->next) {
        ...
        *alloc_size += sizeof(struct vm_map_entry_info) + sizeof(struct vm_object_info)
            + object->resident_page_count * (sizeof(struct vm_page_info) + PAGE_SIZE);
    }

    *buf = malloc(*alloc_size, M_SLSMM, M_NOWAIT);
...
int
vmspace_dump(void **buf, size_t *buf_size, struct vmspace *vms, 
        vm_offset_t start, vm_offset_t end) 
{
    ...
    error = allocate_membuf(buf, &alloc_size, vmmap);
    if (error) return error;
    *buf_size = 0;
    ...
    printf("%lx %lx %lx %lx\n", vms->vm_swrss, vms->vm_tsize, 
            vms->vm_dsize, vms->vm_ssize);
    write_buf(*buf, &vms_info, buf_size, sizeof(struct vmspace_info), alloc_size);

    for (entry = vmmap->header.next; entry != &vmmap->header; entry = entry->next) {
        object = entry->object.vm_object;
        if (object == NULL) continue;
        printf("before\n");
        print_shadow_list(entry->object.vm_object);
        ...
        error = write_buf(*buf, &entry_info, buf_size, 
                sizeof(struct vm_map_entry_info), alloc_size);
        if (error) break;
        ...
        error = write_buf(*buf, &object_info, buf_size, 
                sizeof(struct vm_object_info), alloc_size);
        if (error) break;
        ...
        TAILQ_FOREACH(page, &object->memq, listq) {
            ...
            if (vaddr >= start && vaddr < end) {
                ...
                error = write_buf(*buf, &page_info, buf_size, 
                        sizeof(struct vm_page_info), alloc_size);
                if (error) break;
                
                error = write_buf(*buf, (void*)vaddr, buf_size, PAGE_SIZE, 
                        alloc_size);
                if (error) break;
            }
        }
