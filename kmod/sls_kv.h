...
int slskv_find_unlocked(struct slskv_table *table, uint64_t key, uintptr_t *value);
int slskv_find(struct slskv_table *table, uint64_t key, uintptr_t *value);
int slskv_add_unlocked(struct slskv_table *table, uint64_t key, uintptr_t value);
...
#define KV_FOREACH_POP_UNLOCKED(table, kvkey, kvvalue) \
	...
	while (slskv_pop_unlocked(table, (uint64_t *) &kvkey, (uintptr_t *) &kvvalue) == 0)
...
#define KV_ABORT(iter) do { (slskv_iterabort(&(iter))); } while (0)
