...
int
file_pread(void* addr, size_t len, int fd, size_t off)
{
	int error = 0;
	...
	error = kern_preadv(curthread, fd, &auio, off);
	if (error) {
		printf("Error: kern_readv failed with code %d\n", error);
	}
...
int
file_pwrite(void* addr, size_t len, int fd, size_t off)
{
	int error = 0;
	struct uio auio;
	struct iovec aiov;
	...
	error = kern_pwritev(curthread, fd, &auio, off);
	if (error) {
		printf("Error: kern_writev failed with code %d\n", error);
	}
